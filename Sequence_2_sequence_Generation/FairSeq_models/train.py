...
def train(model_name, epochs, batch_size):
    if model_name == "LSTM":
        ...
        command = "CUDA_VISIBLE_DEVICES=0 fairseq-train {} \
                    --batch-size {} \
                    --arch transformer --max-epoch {}  \
                    --save-dir checkpoints/transformer \
                    --optimizer adam  --dropout 0.3 \
                    --decoder-attention-heads 10 --encoder-attention-heads 10 \
                    --lr 0.0005 --lr-scheduler inverse_sqrt --warmup-updates 4000 --warmup-init-lr 1e-07 \
                    --no-epoch-checkpoints --num-workers 3 \
                    --encoder-embed-path glove.840B.300d.txt --encoder-embed-dim 300 --decoder-embed-dim 300 \
                    --decoder-embed-path glove.840B.300d.txt".format(
            FAIRSEQ_PREPROCESSED_DATASET, batch_size, epochs
        )
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Utility to train the models")
